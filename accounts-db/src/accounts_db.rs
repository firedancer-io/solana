...
impl AccountsDb {
    ...
    fn calculate_accounts_hash_with_verify(
        ...
        if debug_verify {
            ...
            if !success {
                warn!("calculate_accounts_hash_with_verify mismatch. hashes: {}, {}; lamports: {}, {}; expected lamports: {:?}, data source: {:?}, slot: {}", accounts_hash.0, accounts_hash_other.0, total_lamports, total_lamports_other, expected_capitalization, data_source, slot);
                }
        }
        Ok((accounts_hash, total_lamports))
    }
