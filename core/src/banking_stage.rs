...
impl BankingStage {
    ...
    pub fn new(
        ...
        // FIREDANCER: Application name is passed through so we know where to find
        // workspaces.
    ...
    pub fn new_thread_local_multi_iterator(
        ...
        let metric_in_pod = unsafe { solana_firedancer::Pod::join_default(format!("{}_metric_in.wksp", firedancer_app_name)).unwrap() };
        ...
        bank_thread_hdls.extend(
            (0..num_threads)
                .map(|id| {
                    ...
                    let metric_in_pod = metric_in_pod.clone();
                    let in_pod = in_pod.clone();
                    Builder::new()
                        .name(format!("solBanknStgTx{:02}", id + 1))
                        .spawn(move || {
                            unsafe {
                                Self::firedancer_bank_tile(
                                    poh_recorder.as_ref(),
                                    &consumer,
                                    id as u64,
                                    &metric_in_pod,
    ...
    /// FIREDANCER: Implements a run loop for the bank tile, one per thread.
    unsafe fn firedancer_bank_tile(
        poh_recorder: &RwLock<PohRecorder>,
        consumer: &Consumer,
        id: u64,
        metrics_in_pod: &solana_firedancer::Pod,
        in_pod: &solana_firedancer::Pod,
    ) {
        ...
        let in_fseq = FSeq::join::<GlobalAddress>(metrics_in_pod.try_query(format!("fseq_pack_bank_0_bank_{}", id)).unwrap()).unwrap();
        ...
        let cnc = Cnc::join::<GlobalAddress>(metrics_in_pod.try_query(format!("cnc_bank_{}", id)).unwrap()).unwrap();
        ...
        loop {
            // Tell producer that we are no longer busy processing this message.
            in_busy.rx_cr_return(&in_mcache);
            ...
            // The low byte of the signature indicates which bank tile
            // it is for.  The high 7 bytes indicate the slot it is for.
            let sig = in_mcache.sig();
            let target_bank_idx = sig & 0xFF;
            let target_slot = sig >> 8;
            if target_bank_idx != id {
                in_mcache.skip();
                now = rdtsc();
                continue;
            }
            ...
            let working_bank_slot =  bank_start.working_bank.slot();
            if working_bank_slot < target_slot {
                // The PoH recorder just updated the slot number, but we
                // haven't observed it yet. Spin until we do.
                now = rdtsc();
                continue;
            }

            if working_bank_slot > target_slot {
                // The pack tile packed a microblock too late. Drop the transactions.
                warn!("pack tile packed a late microblock for bank tile {}, slot {}, but the bank we have is for slot {}", id, target_slot, working_bank_slot);
                in_mcache.skip();
                now = rdtsc();
                continue;
            }
            ...
            while index < txns.len() {
                let end = usize::min(index + consumer::TARGET_NUM_TRANSACTIONS_PER_BATCH, txns.len());
                    ...
                    match result {
                        ...
                        Err(err) => {
                            match err {
                                ...
                                TransactionError::WouldExceedAccountDataBlockLimit => panic!("pack tile produced invalid block {:#?}", err),
                ...
                if !output.retryable_transaction_indexes.is_empty() {
                    // The only retryable error that wasn't covered in
                    // the match above is a RecorderError that comes
                    // from the PohRecorder having reached max height.
                    // Firedancer doesn't retry transactions, so in this
                    // case, just drop the rest
                    warn!("pack tile produced a microblock too late and it couldn't be fully executed");
                }
                ...
                match output.commit_transactions_result {
                    Err(err) => {
                        match err {
                            ...
                            solana_poh::poh_recorder::PohRecorderError::MinHeightNotReached => unreachable!(), // Not possible, we are using the working bank
                    ...
                    Ok(details) => {
                        for detail in details {
                            match detail {
                                committer::CommitTransactionDetails::Committed { compute_units: _ } => (),
                                committer::CommitTransactionDetails::NotCommitted => (), // This should only happen for transactions that weren't executed either
                            }
                        }
                    }
                }
