...
impl BankingStage {
    ...
    unsafe fn firedancer_bank_tile(
        ...
        loop {
            ...
            if working_bank_slot > target_slot {
                // The pack tile packed a microblock too late
                warn!("Pack tile packed a microblock for bank tile {}, slot {}, but the bank we have is for slot {}", id, target_slot, working_bank_slot);
                in_mcache.skip();
                now = rdtsc();
                continue;
            }
            ...
            while index < txns.len() {
                ...
                if !output.retryable_transaction_indexes.is_empty() {
                    ...
                    warn!("pack tile produced block too late");
                }
