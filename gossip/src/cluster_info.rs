...
impl Node {
    ...
    pub fn new_with_external_ip(
        ...
        // FIREDANCER: The desired TPU and TVU ports are passed in from the config.toml file
        ...
        // FIREDANCER: Correct TVU and TVU forward ports are managed by Firedancer, so these are unused.
        ...
        // FIREDANCER: Publish the port configured in the Firedancer config over gossip, not
        // whatever port Solana Labs manages to bind. Firedancer does not distinguish between
        // TVU and TVU forwards, so we use the same port for both.
