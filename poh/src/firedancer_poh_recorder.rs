...
extern "C" {
    fn fd_ext_poh_initialize(hashcnt_duration_nanos: u64, hashcnt_per_tick: u64, ticks_per_slot: u64, tick_height: u64, last_entry_hash: *const u8, signal_leader_change: *mut c_void);
    ...
    fn fd_ext_poh_begin_leader(bank: *const c_void, slot: u64);
    fn fd_ext_poh_reset(reset_bank_slot: u64, reset_blockhash: *const u8 );
}
...
impl PohRecorder {
    ...
    pub fn new_with_clear_signal(
        ...
        assert_eq!(0, poh_config.target_tick_duration.as_nanos() as u64 % poh_config.hashes_per_tick.unwrap_or(1));
        let hash_duration_nanos: u64 = poh_config.target_tick_duration.as_nanos() as u64 / poh_config.hashes_per_tick.unwrap_or(1) as u64;

        unsafe { fd_ext_poh_initialize(hash_duration_nanos, poh_config.hashes_per_tick.unwrap_or(1), ticks_per_slot, tick_height, last_entry_hash.as_ref().as_ptr(), clear_bank_sender as *mut c_void) };
    ...
    pub fn leader_after_n_slots(&self, _slots: u64) -> Option<Pubkey> {
    ...
    pub fn set_bank(&mut self, bank: Arc<Bank>, _track_transaction_indexes: bool) {
        /* Must be implemented, used by replay stage. */
        let slot = bank.slot();
        let leader_bank: *const Bank = Arc::into_raw( bank );
        unsafe { fd_ext_poh_begin_leader( leader_bank as *const c_void, slot ) };
    }
