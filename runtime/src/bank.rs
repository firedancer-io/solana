...
#[no_mangle]
pub extern "C" fn fd_ext_bank_load_and_execute_txns( bank: *const std::ffi::c_void, txns: *const std::ffi::c_void, txn_count: u64, out_load_results: *mut i32, out_executing_results: *mut i32, out_executed_results: *mut i32, out_load_and_execute_output: *mut *mut std::ffi::c_void ) {
    let txns = unsafe {
    ...
    unsafe { *out_load_and_execute_output = Box::into_raw(load_and_execute_output) as *mut std::ffi::c_void };
}
...
pub extern "C" fn fd_ext_bank_release_thunks( load_and_execute_output: *mut std::ffi::c_void ) {
    let load_and_execute_output = load_and_execute_output as *mut Box<LoadAndExecuteTransactionsOutput>;
    drop(unsafe { Box::from_raw(load_and_execute_output) });
}
